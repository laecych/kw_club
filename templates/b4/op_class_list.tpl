    ...
    <div class="row">
        <div class="col-sm-10">
            ...
            <h4>
                ...
                <{if !$chk_time}>
                    <span class="badge badge-danger"><{$smarty.const._MD_KWCLUB_NON_REGISTRATION_TIME}></span>
                <{/if}>
            </h4>
        </div>
                    ...
                    <li class="vm w1 text-center">
                        <{if $smarty.session.isclubAdmin}>
                            <{if $data.class_ischecked!=1}>
                                <a href="club.php?op=class_enable&class_id=<{$data.class_id}>" class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MD_KWCLUB_CLASS_ENABLE_DESC}>"><{$smarty.const._MD_KWCLUB_CLASS_ENABLE}></a>
                            <{else}>
                                <a href="club.php?op=class_unable&class_id=<{$data.class_id}>" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MD_KWCLUB_CLASS_UNABLE_DESC}>"><{$smarty.const._MD_KWCLUB_CLASS_UNABLE}></a>
                            <{/if}>
                            <a href="club.php?op=class_blank&class_id=<{$data.class_id}>" class="btn btn-sm btn-secondary" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MD_KWCLUB_CLASS_BLANK_DESC}>"><{$smarty.const._MD_KWCLUB_CLASS_BLANK}></a>
                        <{/if}>

                        <{if $smarty.session.isclubAdmin || $uid == $data.class_uid}>
                            <a href="club.php?class_id=<{$data.class_id}>" class="btn btn-sm btn-warning" <{if !$can_operate}>data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MD_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><{$smarty.const._TAD_EDIT}></a>
                            <{if $data.class_regnum == 0}>
                                <a href="javascript:delete_class_func(<{$data.class_id}>);" class="btn btn-sm btn-danger" <{if !$can_operate}>data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MD_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><{$smarty.const._TAD_DEL}></a>
                            <{/if}>
                        <{else}>
                            <{if $data.is_full}>
                                <a href="#" class="btn btn-danger btn-sm disabled" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MD_KWCLUB_NUMBER_OF_APPLICANTS}> <{$data.class_regnum}> <{$smarty.const._MD_KWCLUB_PEOPLE}>"><i class="fa fa-user-plus" aria-hidden="true"></i>
                                    <{$smarty.const._MD_KWCLUB_FULL_REGISTRATION}></a>
                            <{elseif $data.class_regnum >= $data.class_member}>
                                <a href="index.php?op=reg_form&class_id=<{$data.class_id}>&is_full=1" class="btn btn-warning btn-sm"><i class="fa fa-user-plus" aria-hidden="true"></i>
                                    <{$smarty.const._MD_KWCLUB_SIGNUP_TO_MAKE_UP}></a>
                            <{elseif $chk_time}>
                                <a href="index.php?op=reg_form&class_id=<{$data.class_id}>" class="btn btn-primary btn-sm"><i class="fa fa-user-plus" aria-hidden="true"></i>
